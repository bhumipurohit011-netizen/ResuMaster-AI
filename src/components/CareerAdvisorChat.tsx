...
import { saveChatMessage, getUserChatMessages, clearUserChatMessages } from '../lib/storage';
...
const QUICK_PROMPTS = [
  { label: '✨ Improve My Resume', prompt: 'What are the top 3 high-impact bullet points I should rewrite to improve my resume right now?' },
  { label: '📈 Increase ATS Score', prompt: 'Explain exactly why my ATS score is at its current level and give me a step-by-step checklist to raise it by 15 points.' },
  { label: '🎯 Find Skill Gaps', prompt: 'Which high-demand technical skills am I missing for my target role, and what is the fastest way to learn them?' },
  { label: '🚀 Suggest Projects', prompt: 'Suggest 2 impressive portfolio projects I can build to prove my skills to recruiters for this role.' },
  { label: '💼 Match Job Description', prompt: 'How do recruiters and ATS algorithms evaluate job description keyword alignment?' },
  { label: '🎤 Interview Preparation', prompt: 'Based on the experience listed on my resume, what are 3 tough technical interview questions I should prepare for?' },
];
...
How can I help you strengthen your resume and advance your career today?`;
  ...
  // Handle external query trigger (e.g. from Skill gap button)
  useEffect(() => {
  ...
  const sendMessage = async (textToSend: string) => {
    ...
    try {
      const chatContext = {
        ...
        missingSkills: analysis?.missing_skills?.map((s) => s.name) || [],
      };
      ...
      const fallbackMsg: ChatMessage = {
        ...
        resume_id: resumeId,
        role: 'assistant',
        message: "I'm experiencing a temporary network issue. Please check your connection and try again.",
        created_at: new Date().toISOString(),
      };
            ...
            >
              ...
              >
                {isUser ? (
                  ...
                  <div className="prose prose-sm max-w-none text-xs text-slate-800 space-y-2 font-normal">
      ...
      <div className="px-5 py-2.5 bg-white border-t border-slate-100 overflow-x-auto flex items-center gap-2">
        ...
        {QUICK_PROMPTS.map((qp, idx) => (
          <button
            ...
            className="flex-shrink-0 px-2.5 py-1 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 text-slate-700 rounded-lg text-xs font-medium transition-colors"
          >
            {qp.label}
          </button>
        ))}
